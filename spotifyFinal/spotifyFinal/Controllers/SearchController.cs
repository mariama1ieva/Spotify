...
namespace spotifyFinal.Controllers
{
    public class SearchController : Controller
    {
        ...
        public async Task<IActionResult> Search(string searchText)
        {
            ...
            if (!string.IsNullOrEmpty(searchText))
            {
                model = new()
                {
                    ...
                    MusicPlaylists = await _context.MusicPlaylists.Include(m => m.Song).Include(m => m.Playlist).Where(m => m.Playlist.Name.ToLower().Contains(searchText.ToLower()) && !m.SoftDelete).Take(4).ToListAsync()
                };
            }
            else
            {
                model = new()
                {
                    ...
                    MusicPlaylists = await _context.MusicPlaylists.Include(m => m.Song).Include(m => m.Playlist).Where(m => !m.SoftDelete).Take(4).ToListAsync()
                };
            }
        ...
        public async Task<IActionResult> Filter(int id, string searchText)
        {
            ...
            {
                model.Albums = await _context.Albums.Include(m => m.Songs).Where(m => m.Name.ToLower().Contains(searchText.ToLower()) && !m.SoftDelete).Take(4).ToListAsync();
                return PartialView("_Search", model);
            }
            else if (id == 4)
            {
                model.MusicPlaylists = await _context.MusicPlaylists.Include(m => m.Song).Include(m => m.Playlist).Where(m => m.Playlist.Name.ToLower().Contains(searchText.ToLower()) && !m.SoftDelete).Take(4).ToListAsync();
                return PartialView("_Search", model);
            }
